...
async def test_login_success(httpx_mock: HTTPXMock):
    """
    Tests the full successful login flow by mocking API responses.
    """
...
async def test_login_fails_on_bad_credentials(httpx_mock: HTTPXMock):
    """
    Tests that login fails when the authentication server returns an error.
    """
...
def authenticated_client() -> MultisportClient:
    """Provides a client that is already 'logged in' by setting the access token."""
...
async def test_get_card_history(httpx_mock: HTTPXMock, authenticated_client: MultisportClient):
    ...
    history = await authenticated_client.get_card_history(
        product_id, 
