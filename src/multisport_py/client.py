...
class MultisportClient:
    ...
    async def _request_with_retry(self, method: Callable, url: str, **kwargs: Any) -> httpx.Response:
        """
        Performs an HTTP request, retrying with token refresh if a 401 Unauthorized
        status is received.
        """
        try:
