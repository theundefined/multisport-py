...
class MultisportClient:
    ...
    async def _authenticate_step1(self) -> str:
        """
        Performs the initial login flow to obtain an authorization code.
        This involves:
        1. Making a GET request to the OpenID Connect /auth endpoint to get dynamic parameters.
        ...
        login_init_url = (
            ...
            f"nonce={self._generate_code_verifier(16)}&" # Nonce can be a simpler random string
        ...
        if not match:
            logger.error(f"Could not find login action URL in the HTML response.")
        ...
        if login_response.status_code != 302:
            logger.error(f"Login POST failed. Expected status 302, got {login_response.status_code}. Response: {login_response.text[:200]}")
            raise ValueError(f"Authentication failed: Unexpected status code {login_response.status_code}.")

        redirect_location = login_response.headers.get("Location")
        if not redirect_location:
            logger.error(f"No redirect location found after login POST. Response status: {login_response.status_code}, text: {login_response.text[:200]}")
        ...
        logger.error(f"Failed to extract authorization code from redirect fragment: {redirect_location}")
    ...
    async def login(self):
        ...
        try:
            ...
            logger.exception("MultiSport login failed:") # Use exception for full traceback
            raise

    async def get_user_info(self) -> Dict[str, Any]:
        """Fetches user information from the Keycloak userinfo endpoint."""
        if not self.access_token:
            raise ValueError("Not logged in. Call client.login() first.")
        
        headers = {"Authorization": f"Bearer {self.access_token}"}
        response = await self.http_client.get(
            f"{self.AUTH_BASE_URL}protocol/openid-connect/userinfo",
            headers=headers
        )
    ...
    async def get_authorized_users(self) -> Dict[str, Any]:
        """Fetches authorized users from the BAM API. This should contain product IDs."""
        if not self.access_token:
            raise ValueError("Not logged in. Call client.login() first.")
        
        headers = {"Authorization": f"Bearer {self.access_token}", "Accept-Language": "pl"} # Added Accept-Language from curl
        response = await self.http_client.get(
            f"{self.API_BASE_URL}bam/core/v1/authorized/users",
            headers=headers
        )
        response.raise_for_status()
        return response.json()
    ...
    async def get_card_limits(self, product_id: str) -> Dict[str, Any]:
        """Fetches card limits for a given product ID."""
        if not self.access_token:
            raise ValueError("Not logged in. Call client.login() first.")
        
        headers = {"Authorization": f"Bearer {self.access_token}", "Accept-Language": "pl"}
        response = await self.http_client.get(
    ...
    async def get_card_history(self, product_id: str, date_from: Optional[str] = None, date_to: Optional[str] = None) -> Dict[str, Any]:
        """
        Fetches card entry history for a given product ID.
    ...
    async def get_relations(self) -> Dict[str, Any]:
        """Fetches related cards and users."""
        if not self.access_token:
            raise ValueError("Not logged in. Call client.login() first.")
        
        headers = {"Authorization": f"Bearer {self.access_token}", "Accept-Language": "pl"}
        response = await self.http_client.get(
            f"{self.API_BASE_URL}bam/relations/v1/authorized/relations",
            headers=headers
        )
        response.raise_for_status()
        return response.json()
